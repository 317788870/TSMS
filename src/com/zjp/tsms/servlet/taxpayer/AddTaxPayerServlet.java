...
/**
 * @author zjp
 * @date 2017年11月8日 下午2:49:09
 */
@WebServlet("/manage/addTaxPayer.json")
public class AddTaxPayerServlet extends BaseServlet {
	...
	protected void excuteRequest(HttpServletRequest request, HttpServletResponse response)
		...
		payer.setRecordDate(new SimpleDateFormat("yyyy-MM-dd").format(new Date()));
		boolean flag = TaxPayerDao.getInstance().addTaxPayer(payer);
		if (flag) {
			json.put("msg", " 添加成功");
